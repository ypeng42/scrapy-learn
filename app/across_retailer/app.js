        ...
        $.each(value, function(index, obj) {
            if (obj.product_name.toLowerCase().indexOf(filter_num) != -1
              && obj.product_name.toLowerCase().indexOf(filter_unit) != -1) {
                $ul.append(
                    "<li><input type='checkbox' value='" + obj.product_name + '#' + obj.retailer + "'/>" +
...
price_across_retailer = [];

curr_category = "";

// Pick a category
$("#category_pick").on('change', function() {
    // Clear graph and hide table 
    $("#across_retailer").empty();
    $('#product_table').hide();

    // Also hide the buttons and checkboxes and other
    hide_buttons();
...
$("#brand_pick").on('change', function() {
    ...
    $.ajax({
        ...
        success: function(data) {
          ...
          $.each(data, function(index, value) {
              if (!(value.retailer in checkboxes_list)) {
                  checkboxes_list[value.retailer] = [];
                  checkboxes_list[value.retailer].push({
...
$('body').on('click', '.submit_button', function () {
    ...
    $.ajax({
        url: 'get_price.php',
        type: 'post',
        data: {'plist' : plist, "retailer_list" : Object.keys(checkboxes_list)},
        dataType: "json",

        success: function(data) { 
          $.each(data['price_info'], function(index, value) {
              if (value.average != 0) {
                price_across_retailer.push(new PriceInfo(value.retailer, value.average,
                  value.variance));
              }
          });

          show_price(price_across_retailer, "#across_retailer");
